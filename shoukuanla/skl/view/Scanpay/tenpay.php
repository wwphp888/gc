...
<html>
<head>
<title>QQ钱包 - 网上支付 安全快速！</title>
<meta charset="utf-8" />
<meta name="keywords" content="QQ钱包扫码付款" />
<meta name="description" content="QQ钱包扫码付款" />
<meta http-equiv="X-UA-Compatible" content="IE=edge" />
<link charset="utf-8" rel="stylesheet" href="<?php echo SKL_STATIC_PATH; ?>css/sklcss.css" media="all" />
<script charset="utf-8" src="<?php echo SKL_PUBLIC_PATH; ?>js/jquery.min.js"></script>
</head>

<style>
.copy{
	background-image: url(<?php echo SKL_STATIC_PATH; ?>images/tencopy.jpg); background-repeat: no-repeat; height: 430px; width: 300px; margin-right: auto; margin-left: auto; text-align: left; }
.inputEmail { width: 30px; padding-left: 35px; padding-top: 153px; }
.inputsty{ font-size: 18px; width: 230px; height: 28px; }
    ...
    .alipay-logo {
        ...
        background-image: url(<?php echo SKL_STATIC_PATH; ?>images/tenlogo.png);
    }
</style>
...
<div class="topbar">
    <div class="topbar-wrap fn-clear">
        		...
        		<span class="topbar-link-first">你好，欢迎使用QQ钱包付款！</span>
		    </div>
</div>
...
<div class="mi-notice mi-notice-error mi-notice-titleonly order-timeout-notice" id="alertNotice">
    <div class="mi-notice-cnt">
        <div class="mi-notice-title">
            ...
            <h3>重要提示：扫码付款时必须输入指定<?php if($post['is_float'] == '1'){ echo '金额('.$post['short_order'].')'; }else{  echo '金额('.$post['money'].')和备注('.$post['short_order'].')'; }						
						?>，否则会充值失败哦。</h3>
...
<div id="order" data-role="order" class="order order-bow">
          ...
          <?php echo empty($post['web_order']) ? '系统订单号：'.$post['sys_order']: '网站订单号：'.$post['web_order']; ?> <br/>
...
距离该订单过期还有：
    ...
    <strong id="second_show">0秒</strong>
                                                ...
                                                <span class="payAmount-area" id="J_basePriceArea">
                                                     <strong class=" amount-font-22 "><?php if($post['is_float'] == '1'){ echo $post['short_order']; }else{ echo $post['money']; }?></strong> 元
                
        </span>
            </div>

            
            <a style="letter-spacing:2px;" id="J_OrderExtTrigger" class="order-ext-trigger" href="<?php echo $post['return_url']; ?>" seed="order-detail-more" data-role="J_oderDetailMore">
                返回网站
            </a>
    ...
    <div class="cashier-center-container">
        ...
        <div data-module="excashier/login/2016.08.01/loginPwdMemberT" id="J_loginPwdMemberTModule" class="cashiser-switch-wrapper fn-clear" >
            ...
            <!-- 扫码支付页面 -->
            <div <?php if(!$is_remittance){ echo 'style="margin-left:30px;"'; } ?> class="cashier-center-view view-qrcode fn-left" id="J_view_qr">
...
.area-split {
    ...
    background-image: url(<?php echo SKL_STATIC_PATH; ?>images/t1pspfxixsxxxxxxxx.png);
    background-repeat: no-repeat;
}
...
<div data-role="qrPayArea" class="qrcode-integration qrcode-area" id="J_qrPayArea">
        ...
        <div class="ft-center">扫一扫付款（元）</div>
				...
				<?php 
				  if($post['is_float'] == '1'){
            echo '<div class="ft-center qrcode-header-money">指定金额：'.$post['short_order'].'</div>';

					}else{
	          
						echo '<div class="ft-center qrcode-header-money">指定金额：'.$post['money'].'</div>';
					  echo '<div class="ft-center qrcode-header-money">指定备注：'.$post['short_order'].'</div>';				
					}
				?>
...
<?php 
echo '<img id="qrimage" src="'.$qrcodePath.'" width="230" title="'.$post['in_path'].'" />';
?>           
     ...
     <div id="loading" class="ui-loading qrcode-loading" data-role="qrPayImgLoading">加载中</div>
        ...
        <div class="qrcode-img-explain fn-clear">
            <img class="fn-left" src="<?php echo SKL_STATIC_PATH; ?>images/t1bdtfxfdixxxxxxxx.png" alt="扫一扫标识">
            <div class="fn-left">打开手机QQ<br>扫一扫开始付款</div>
        </div>
        ...
        <div data-role="qrPayExplain" class="qrcode-explain fn-hide">
            <a href="https://mobile.alipay.com/index.htm" class="qrcode-downloadApp" data-boxUrl="https://cmspromo.alipay.com/down/new.htm" data-role="dl-app" target="_blank" seed="NewQr_qr-pay-download">首次使用请下载手机QQ</a>
        </div>
...
<div class="qrguide-area" id="J_qrguideArea" seed="NewQr_animationClick">
    <img id="qrguideimg" src="<?php echo SKL_STATIC_PATH; ?>images/tenalert.png" class="qrguide-area-img active">
    <img src="<?php echo SKL_STATIC_PATH; ?>images/t1asfgxdtnxxxxxxxx.png" class="qrguide-area-img background">
</div>
 ...
 <!-- 点击切换区域 -->
<div <?php if(!$is_remittance){ echo 'style="display:none;"'; } ?> class="view-switch qrcode-show fn-left" id="J_viewSwitcher" unselectable="on" onselectstart="return false;" seed="NewQr_viewSwitch">
    ...
    <div class="switch-tip switch-qrcode-tip " id="J_tip_qr">
        ...
        <div class="switch-tip-icon-wrapper">
            ...
            <img class="switch-tip-icon-img" src="<?php echo SKL_STATIC_PATH; ?>images/tenlogo.png" alt="QQ钱包图标" width="50" height="17">
        </div>
        <a class="switch-tip-btn" href="javascript:void(0)">&lt;&nbsp;手动转账</a>
    </div>

    <div class="switch-tip switch-pc-tip fn-hide" id="J_tip_pc">
        <div class="switch-tip-font">试试手机QQ钱包</div>
        <div class="switch-tip-icon-wrapper">
            <i class="switch-tip-icon iconfont" title="手机">&#xF033;</i>
            <img class="switch-tip-icon-img" src="<?php echo SKL_STATIC_PATH; ?>images/qqqier.png" alt="手机QQ钱包图标" width="30" height="30">
        </div>
        <a class="switch-tip-btn" href="javascript:void(0)">扫一扫付款&nbsp;&gt;</a>
            ...
            <!-- PC 付款登录页面 -->
            <div <?php if(!$is_remittance){ echo 'style="display:none;"'; } ?> class="cashier-center-view view-pc fn-left" id="J_view_pc">
...
<div class="loginBox">
    ...
    <div class="login-title-area">
        <div class="login-title">
            <div class="login-title-left" style="text-align: center; float:none;">
                <p class="lt-text ft-yh">打开手机QQ将以下收款信息输入到QQ钱包转账页面，然后点击确定转账</p>
            </div>
...
<div class="copy">
 ...
 <div class="inputEmail"> <input style="text-align: center;" onClick="this.select();" name="Email" class="inputsty" name="input" type="text" value="<?php echo $post['email']; ?>" readonly> </div>

 <div class="inputEmail" style="padding-top:66px;"> <input onClick="this.select();" style="height: 40px;font-size: 30px;" class="inputsty" name="money" type="text" value="<?php if($post['is_float'] == '1'){ echo $post['short_order']; }else{ echo $post['money']; } ?>" readonly> </div>
 
 <div class="inputEmail" style="padding-top:24px;padding-left: 18px;"> <input onClick="this.select();" style="height: 30px;width: 264px;" class="inputsty" name="text" type="text" value="<?php if($post['is_float'] != '1'){ echo $post['short_order']; } ?>" readonly> </div>
</div>
            </div>
...
<div class="copyright">
      <label style="color:#F33;">温馨提示：付款成功后1-20秒到账请耐心等候，如果超时未到账请联系网站客服处理</label>
  </div>
...
<div id=partner><img alt=合作机构 src="<?php echo SKL_STATIC_PATH; ?>images/2r3ckfrkqs.png"></div>	
 ...
 $.get("<?php echo skl_U('Querystatus/index'); ?>", { sys_order: sys_order,timestamp:new Date().getTime() },
  function(data){   
     ...
     if(data.skl_state == '1'){
			...
			$("#qrimage").attr("src","<?php echo SKL_STATIC_PATH; ?>images/failure.png"); 
		  upTimeout();
...
$('#alertNotice').show(200,function(){
	window.setTimeout("$('#alertNotice').hide(1000*3)",1000*60);
});
